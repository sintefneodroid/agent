...
def wiener(x0, n, dt, delta, out=None):
  ...
  r = norm.rvs(size=x0.shape + (n,), scale=delta * sqrt(
      dt))  # For each element of x0, generate a sample of n numbers from a normal distribution.
  ...
  np.cumsum(r, axis=-1,
            out=out)  # This computes the Brownian motion by forming the cumulative sum of the random samples.
