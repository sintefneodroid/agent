...
from neodroidagent.utilities.specifications.exploration_specification import (
...
REPLAY_MEMORY_SIZE = int(3e10)
INITIAL_OBSERVATION_PERIOD = int(3e4)
DISCOUNT_FACTOR = 0.8
...
# Optimiser
OPTIMISER_SPEC = GDKC(torch.optim.Adam, NOD(lr=3e-5, weight_decay=3e-6))

# OPTIMISER_SPEC = GDKC(torch.optim.SADGD,
#                      NOD(lr=3e-5,
#                          weight_decay=3e-6,
#                          momentum=0.9
# eps=3e-3
...
SCHEDULER_SPEC = GDKC(
    torch.optim.lr_scheduler.StepLR, NOD(step_size=ROLLOUTS // 8, gamma=0.3)
)
