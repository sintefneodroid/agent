...
"""
    ...
    def __init__(
        ...
        optimiser_spec=GDKC(
            torch.optim.RMSprop, alpha=0.9, lr=0.0025, eps=1e-02, momentum=0.0
        ),
        scheduler_spec=None,
        **kwargs,
    ):
    ...
    def __build__(
        ...
        **kwargs,
        ...
        if print_model_repr:
            sprint(f"Value model: {self._value_model}", highlight=True, color="cyan")
        ...
        **kwargs,
        ...
        if metric_writer:
            metric_writer.scalar(
                "Current Eps Threshold", self._current_eps_threshold, self._sample_i
            )

        if (s and self._sample_i > self._initial_observation_period) or no_random:
