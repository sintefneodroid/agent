...
from neodroidagent.agents.torch_agents.model_free.off_policy.value_agent import (
    ValueAgent,
)
...
"""
    ...
    def _remember(self, *, state, action, signal, next_state, terminated, **kwargs):
        self._memory_buffer.add_transition(
            state, action, signal, next_state, terminated
        )
...
"""
    ...
    def _update(self, *, metric_writer=MockWriter(), **kwargs):
        ...
        if (
            ...
            and self.update_i % self._sync_target_model_frequency == 0
        ):
            self._target_value_model = copy_state(
                target=self._target_value_model, source=self._value_model
            )
            if metric_writer:
                metric_writer.scalar(
                    "Target Model Synced", self.update_i, self.update_i
                )
...
"""
        ...
        states = to_tensor(
            batch.state, dtype=self._state_type, device=self._device
        ).transpose(0, 1)
...
def dqn_run(rollouts=None, skip=True, connect_to_running=True):
    ...
    session_entry_point(
        ...
        session=ParallelSession(
            procedure=OffPolicyEpisodic, auto_reset_on_terminal_state=True
        ),
