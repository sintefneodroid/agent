...
"""
    ...
    def _update(
        self, transitions=None, *args, metric_writer: Writer = MockWriter(), **kwargs
    ) -> None:

        adv_trans = self.back_trace_advantages(transitions)
        dataset = AdvDisDataset(adv_trans)
        loader = DataLoader(
            dataset, batch_size=len(dataset) // self._mini_batches, shuffle=True
        )
    ...
    def take_n_steps(
        ...
        **kwargs,
        ...
        transitions = []
        terminated = False
        T = tqdm(range(1, n + 1), f"Step #{self._sample_i} - {0}/{n}", leave=False)
        for t in T:
    ...
    def back_trace_advantages(self, transitions):
        ...
        discounted_signal = discount_signal(
            sig.transpose(0, 1).detach().to("cpu").numpy(), self._discount_factor
        ).transpose()
    ...
    def evaluate(
        self, batch: AdvantageDiscountedTransition, discrete: bool = False, **kwargs
    ):
        # region Tensorise
...
def ppo_run(rollouts=None, skip: bool = True):
    ...
    session_entry_point(
        ...
        C,
        session=ParallelSession(
            StepWise, auto_reset_on_terminal_state=True, environment_type=True
        ),
        parse_args=False,
        skip_confirmation=skip,
    )
