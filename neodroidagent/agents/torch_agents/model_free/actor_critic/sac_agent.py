...
"""
    ...
    def evaluate(self, state, epsilon=1e-6):
        ...
        normal = Normal(0, 1)
        z = normal.sample()
        action = torch.tanh(mean + std * z.to(global_torch_device()))
        log_prob = Normal(mean, std).log_prob(
            mean + std * z.to(global_torch_device())
        ) - torch.log(1 - action.pow(2) + epsilon)
        return action, log_prob, z, mean, log_std

    def get_action(self, state):
        # Then to get the action we use the reparameterization trick
        state = torch.FloatTensor(state).unsqueeze(0).to(global_torch_device())
        mean, log_std = self.forward(state)
        std = log_std.exp()
        ...
        z = normal.sample().to(global_torch_device())
...
"""
    ...
    def models(self) -> Dict[str, Architecture]:
        return {
            "critic1": self.q1,
            "critic2": self.soft_q_net1,
            "actor": self.policy_net,
        }
    ...
    def __build__(
        ...
        **kwargs,
        ...
        self.q1 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(
            global_torch_device()
        )
        self.q1_target = SoftQNetwork(state_dim, action_dim, hidden_dim).to(
            global_torch_device()
        )
        self._update_target(
            target_model=self.q1_target, source_model=self.q1, copy_percentage=1
        )

        self.q2 = SoftQNetwork(state_dim, action_dim, hidden_dim).to(
            global_torch_device()
        )
        self.q2_target = SoftQNetwork(state_dim, action_dim, hidden_dim).to(
            global_torch_device()
        )
        self._update_target(
            target_model=self.q2_target, source_model=self.q2, copy_percentage=1
        )
        ...
        self.policy_net, self.policy_optimizer, frame_idx, rewards = load_checkpoint(
            self.policy_net, self.policy_optimizer, SAC_STATE_PATH
        )
    ...
    def _update(
        ...
        **kwargs,
        ...
        state = torch.FloatTensor(state).to(global_torch_device())
        next_state = torch.FloatTensor(next_state).to(global_torch_device())
        action = torch.FloatTensor(action).to(global_torch_device())
        reward = torch.FloatTensor(reward).unsqueeze(1).to(global_torch_device())
        terminals = (
            torch.FloatTensor(np.float32(terminals))
            .unsqueeze(1)
        ...
        predicted_new_q_value = torch.min(q1_target_value, q2_target_value)
    ...
    def update_targets(self):
        self._update_target(
            target_model=self.q1_target, source_model=self.q1, copy_percentage=soft_tau
        )

        self._update_target(
            target_model=self.q2_target, source_model=self.q2, copy_percentage=soft_tau
        )
...
if __name__ == "__main__":
    ...
    def train(max_frames=500000, max_steps=500, batch_size=128, start_episode=0):
        ...
        while frame_idx < max_frames:
            ...
            for step in range(max_steps):
                ...
                if len(agent._memory) > batch_size:
                    agent.update(
                        batch_size
                    )  # network updates in each run of the inner loop after recording the buffer
...
"""
            ...
            anim = animation.FuncAnimation(
                pyplot.gcf(), animate, frames=len(frames), interval=50
            )
            display(anim)
