...
"""
    ...
    def __init__(
        ...
        actor_optimiser_spec: GDKC = GDKC(
            constructor=torch.optim.Adam, lr=3e-4, weight_decay=3e-3, eps=3e-2
        ),
    ...
    def __build__(
        ...
        **kwargs,
        ...
        if print_model_repr:
            sprint(f"Critic: {self._critic}", highlight=True, color="cyan")
            sprint(f"Actor: {self._actor}", highlight=True, color="cyan")
    ...
    def rollout(
        ...
        **kwargs,
        ...
        T = tqdm(
            count(1), f"Rollout #{self._update_i}", leave=False, disable=not render
        )
        for t in T:
            self._sample_i += 1
    ...
    def remember(
        self, state, action, signal, successor_state, terminated, *args, **kwargs
    ) -> None:
        return self._memory.add_transition(
            state, action, signal, successor_state, terminated
        )
