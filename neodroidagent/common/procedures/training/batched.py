...
__all__ = ["Batched"]
__doc__ = "Collects agent experience in a batched fashion"
...
"""
        with torch.autograd.detect_anomaly():
            with TensorBoardPytorchWriter(log_directory) as metric_writer:
                state, *_ = self.environment.reset().to_gym_like_output()
                ...
                for i in B:
                    ...
                    for _ in S:
                        ...
                        action, action_log_prob, value_estimate, *_ = self.agent.sample(
                            state
                        )
                        ...
                        (
                            ...
                            terminated,
                            *_,
                        ) = self.environment.react(action).to_gym_like_output()
                        ...
                        if stat_frequency and i % stat_frequency == 0:
                            metric_writer.scalar(
                                "Signal", numpy.sum(batch_signal).item()
                            )

                    # only calculate value of next state for the last step this time
                    *_, self.agent._last_value_estimate, _ = self.agent.sample(
                        successor_state, no_random=True
                    )

                    if len(transitions) > 100:
                        self.agent.remember(
                            transitions=transitions, metric_writer=metric_writer
                        )
                        if train_agent:
                            self.agent.update()
                        else:
