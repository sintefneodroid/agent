...
from warg.kw_passing import super_init_pass_on_kws, passes_kws_to
...
class ParallelSession(EnvironmentSession):
  ...
  def __call__(self,
    ...
    listener = add_early_stopping_key_combination(
      self._procedure.stop_procedure,
    ...
    try:
      ...
      resume = proc(**kwargs)
      if resume and 'stats' in resume:
        resume.stats.save(project_name=kwargs.project,
                          config_name=kwargs.config_name,
                          directory=kwargs.log_directory)
    except KeyboardInterrupt:
      pass
    time_elapsed = time.time() - session_start_timestamp
    if listener:
      listener.stop()

    end_message = f'Session done, time elapsed: {time_elapsed // 60:.0f}m {time_elapsed % 60:.0f}s'
    line_width = 9
...
if __name__ == '__main__':
  ...
  from neodroidagent.agents.model_free.on_policy.pg_agent import PGAgent
